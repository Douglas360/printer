...
const columns = [
  ...
  { id: 'date_created', label: 'Aberto em', minWidth: 1, align: 'center' },
  { id: 'subject', label: 'Assunto', minWidth: 1, align: 'center' },
  { id: 'status', label: 'Status', minWidth: 1, align: 'center' },
  { id: 'print', label: 'Imprimir', minWidth: 1, align: 'center' },
...
export default function TableResult() {
  ...
  const [detailTicket, setDetailTicket] = useState<DetailTicketProps[]>([])


  useEffect(() => {
    async function loadTicket() {

      await api.get('/print').then((response) => {
        setTicket(response.data)

        // console.log(response.data)
        //console.log(response.data[0].threads[0].body)
  ...
  const handlePrint = (id: number, subject: String, date_created: String, threads: String) => {

    pdfMake.vfs = pdfFonts.pdfMake.vfs

    const numberTicket = id
    const subjectTicket = subject
    setDetailTicket(htmlToPdfmake(threads))
    const dateCreated = date_created

    const dataSplit = dateCreated.split("T")[0]
    const ano = dataSplit.split("-")[0];
    const mes = dataSplit.split("-")[1];
    const dia = dataSplit.split("-")[2];

    const dataConvertida = `${dia}/${mes}/${ano}`

    // const detailTicket = DetailTicket[0].text
      ...
      content: [
...

        {
          ...
          table: {
            ...
            body: [
              [{ text: 'CHAMADO TECNICO', style: 'tableHeader', alignment: 'center' }, { text: 'DATA', style: 'tableHeader', alignment: 'center' }, { text: 'HORA INICIAL', style: 'tableHeader', alignment: 'center' }, { text: 'HORA FINAL', style: 'tableHeader', alignment: 'center' }],

              [{ text: 'Abertura', style: 'tableHeader', alignment: 'center' }, { text: `${dataConvertida}`, alignment: 'center' }, '', ''],
              [{ text: 'Atendimento', style: 'tableSubHeader', alignment: 'center' }, '', '', ''],
              [{ text: 'Finalização', style: 'tableSubHeader', alignment: 'center' }, '', '', ''],
...

        {
          ...
          table: {
            ...
            body: [
              [{ text: 'NATUREZA: contrato', style: 'tableHeader', }, { text: 'Garantia Até:', style: 'tableHeader' }],
        ...
        },
        {
          ...
          table: {
            ...
            body: [
              [
                { text: 'Defeito Informado', style: 'tableHeader' },
                {
                  text: detailTicket, style: 'detail'
                },
        ...
        },
        {
          ...
          table: {
            ...
            body: [
              [{ text: 'Análise e Correção:', style: 'tableHeader' },]
            ]
          }
        },
        //{text:'Nº OS: ',bold:true,margin:5}
        {
          table: {
            body: [
              [{ text: `Nº OS: `, border: [false, false, false, false] }, { text: `${numberTicket}`, border: [false, false, false, false], style: 'number' }]
            ]
          }
        },

        {
          ...
          table: {
            ...
            body: [
              [{ text: 'LOCAL: ', style: 'tableHeader', }, { text: `${subjectTicket}`, style: 'subject' }],
              [{ text: 'OBSERVAÇÃO: ', style: 'tableHeader', border: [true, false, false, true] }, { text: '', border: [false, true, true, true] }],
        ...
        },
        {
          ...
          table: {
            ...
            body: [
              [{ text: 'STATUS', style: 'tableHeader', }, { text: ' (  ) SERVIÇO PENDENTE', style: 'subject' }, { text: ' (  ) SERVIÇO CONCLUÍDO', style: 'subject' }],
        ...
        },
        {
          ...
          table: {
            ...
            body: [
              [{ text: 'Aceitação: Declaro que os serviços constantes do presente relatório técnico forma efetivamente prestados. Tendo sido dados como aceitos por mim nesta data.', style: 'subject', colSpan: 3, border: [true, true, true, false] }, {}, {},],
              [{ text: '__________________________________', alignment: 'left', border: [true, false, false, false] }, { text: '', border: [false, false, false, false] }, { text: '___________________________________________', alignment: 'right', border: [false, false, true, false] }],
              [{ text: 'TECNICO', alignment: 'center', border: [true, false, false, true] }, { text: '', border: [false, false, false, true] }, { text: 'CLIENTE', alignment: 'center', border: [false, false, true, true] }],
        ...
        <TableBody>
          {ticket.map((row) => (
            ...
            >
              ...
              {/*<TableCell align="center">{row.threads[0]['body']}</TableCell>*/}

              <TableCell align="center">
                <Button variant="contained" onClick={() => handlePrint(row.id, row.subject, row.date_created, row.threads[0]['body'])}>
                  <PrintIcon />
                </Button>
              </TableCell>
            </TableRow>
